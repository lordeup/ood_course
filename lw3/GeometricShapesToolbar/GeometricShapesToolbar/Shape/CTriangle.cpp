...
CTriangle::CTriangle(sf::ConvexShape& shape, const sf::Vector2f& position)
	: CShapeDecorator(shape)
	, m_triangle(shape)
{
	m_points = GetPoints(position);
	m_triangle.setPointCount(m_points.size());

	m_position = position - GetMidpoint();
...
void CTriangle::Initialization()
{
	m_sides = GetSides();
	SetFillColor(YELLOW_COLOR);
...
std::vector<sf::Vector2f> CTriangle::GetPoints(const sf::Vector2f& position) const
{
	std::vector<sf::Vector2f> points;
	const float differencePoints = 100.f;

	points.push_back({ position.x - differencePoints, position.y + differencePoints });
	points.push_back({ position.x, position.y - differencePoints });
	points.push_back({ position.x + differencePoints, position.y + differencePoints });
